...
import App from './App';
import path from 'case-sensitive-paths-webpack-plugin'
import fs from 'fs';
...
const manifest =JSON.parse(
  fs.readFileSync(path.resolve('./build/asset-manifest.js'), 'utf8')
);

const chunks = Object.keys(manifest.files)
  .filter(key=> /chunk\.js$/.exec(key)) // chunk.js로 끝나는 키 찾아서
  .map(key => `<script src="${manifest.files[key]}"></script>`) // 스크립트 태그를 변환하고
  .join(''); //합침
...
preloadContext.done = true;
  ...
  //JSON을 문자열로 변환하고 악성 스크립트가 실행되는 것을 방지 하기 위해 <를 치환 처리
...
app.use(serve); //순서가 중요하다. serverRouter 전에 위치 해야한다.
